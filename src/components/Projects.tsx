        ...
        <div className={`grid ${layout === 'minimal' ? 'grid-cols-1 max-w-4xl mx-auto' : 'md:grid-cols-2'} gap-8 font-sans`}>
          {PROJECTS.map((project, i) => (
            <motion.div
              ...
              className="h-full"
            >
              <ThreeDCard className={`glass rounded-[2.5rem] overflow-hidden group h-full flex flex-col justify-between ${layout === 'minimal' ? 'md:flex-row' : ''}`}>
                <div>
                  ...
                  <div className="p-8">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="text-xl font-bold tracking-tight text-text-main group-hover:text-brand-blue transition-colors">{project.title}</h3>
                ...
                <div className="px-8 pb-8 pt-4 flex items-center justify-between gap-4 mt-auto border-t border-card-border">
                  {project.github !== "#" ? (
                    <a
                      href={project.github}
                      className="text-xs text-text-muted hover:text-text-main flex items-center gap-2 transition-colors cursor-pointer"
                    >
                      <Github className="w-4 h-4" />
                      Source Code
                    </a>
                  ) : (
                    <span className="text-xs text-text-dim">Source: Private Repo</span>
                  )}
                  {project.link !== "#" && (
                    <a
                      href={project.link}
                      ...
                      className="text-xs font-bold text-brand-blue hover:text-brand-purple flex items-center gap-1 transition-colors cursor-pointer"
                    >
